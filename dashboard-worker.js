...
function switchFinanceTab(tab) {
  ...
  methodsBtn.style.color = tab === "methods" ? "#5DCAA5" : "";
  methodsBtn.style.fontWeight = tab === "methods" ? "600" : "";
  txBtn.style.color = tab === "transactions" ? "#5DCAA5" : "";
  txBtn.style.fontWeight = tab === "transactions" ? "600" : "";
...
document.getElementById("financeForm")?.addEventListener("submit", async function(e) {
  ...
  } catch (e) { console.error("Finance save error:", e); alert("Something went wrong."); }
...
async function loadFinanceTransactions() {
  ...
  try {
    ...
    document.getElementById("finStatsRow").innerHTML = `
      <div class="stat-box"><div class="num">GHS ${total.toLocaleString()}</div><div class="label">Total earned</div></div>
    ...
    chartEl.innerHTML = keys.map(key => {
      ...
      return `<div style="flex:1; display:flex; flex-direction:column; align-items:center; gap:4px; height:100%; justify-content:flex-end;">
        <span style="font-size:10px; color:rgba(255,255,255,0.3);">GHS${val}</span>
        <div style="width:100%; max-width:40px; height:${pct}%; background:linear-gradient(180deg,#5DCAA5,rgba(93,202,165,0.3)); border-radius:4px 4px 0 0; transition:height 0.4s;"></div>
        <span style="font-size:10px; color:rgba(255,255,255,0.25);">${label}</span>
      </div>`;
    }).join("");
    // Render transactions
    ...
    container.innerHTML = d.transactions.map(t => {
      ...
      return `<div class="profile-card" style="flex-direction:row; align-items:center; gap:12px; margin-bottom:8px; padding:12px 16px;">
        <div style="flex:1;">
          <p style="font-size:14px; font-weight:500; color:#fff;">${escapeHtml(t.facility_name)}</p>
          <p style="font-size:12px; color:rgba(255,255,255,0.3);">${escapeHtml(t.role_needed)} · ${t.shift_date || "—"}</p>
        </div>
        <div style="text-align:right;">
          <p style="font-size:14px; font-weight:600; color:#5DCAA5;">GHS ${t.amount.toLocaleString()}</p>
          <span style="font-size:11px; color:${statusColor};">${t.payout_status === "paid" ? "Paid" : t.payout_status === "failed" ? "Failed" : "Pending"}</span>
        </div>
      </div>`;
    }).join("");
